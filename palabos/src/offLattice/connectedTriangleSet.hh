...
void ConnectedTriangleSet<T>::computeVertexAreaAndUnitNormal(plint iVertex, T& area,
        Array<T,3>& unitNormal, std::vector<Array<T,3> > *newVertices) const
{
    ...
    for (plint iTriangle = 0; iTriangle < (plint) trianglesOnVertex[iVertex].size(); iTriangle++) {
        ...
        computeTriangleAreaAndUnitNormal(globalTriangle, triangleArea, triangleNormal, newVertices);
        area += triangleArea;
        ...
        Array<T,3>& unitNormal, std::vector<Array<T,3> > *newVertices) const
{
    ...
    if (workingVertices == 0) {
        ...
        PLB_ASSERT(workingVertices->size() == vertices.size());
    }

    plint v0 = triangles[iTriangle][0];
    plint v1 = triangles[iTriangle][1];
    plint v2 = triangles[iTriangle][2];
...
template<typename T>
void ConnectedTriangleSet<T>::writeOFF(std::string fname, std::vector<Array<T,3> > *newVertices) const
{
    if (global::mpi().isMainProcessor()) {
        ...
        for (plint iVertex = 0; iVertex < numVertices; iVertex++) {
            fprintf(fp, "% .10e % .10e % .10e\n", (double) (*workingVertices)[iVertex][0],
                    (double) (*workingVertices)[iVertex][1],
                    (double) (*workingVertices)[iVertex][2]);
        }
        for (plint iTriangle = 0; iTriangle < numTriangles; iTriangle++) {
