...
template<typename T>
void BubbleHistory3D<T>::updateBubblePressure(MultiScalarField3D<T>& outsideDensity, T rhoEmpty, T alpha, T beta)
{
    applyProcessingFunctional (
            new UpdateBubblePressure3D<T>(bubbles, rhoEmpty, alpha, beta),
        ...
        T newBubbleVolumeCorrection, std::map<plint,BubbleInfo3D>& newBubbles, std::map<plint,plint>& newToFinal )
{
    ...
    /// CHANGES
    /*--*/static const T cutoffVolume = 2.0;
...
template<typename T>
UpdateBubblePressure3D<T>::UpdateBubblePressure3D(std::map<plint,BubbleInfo3D> const& bubbles_, T rho0_, T alpha_, T beta_)
    : bubbles(bubbles_),
      rho0(rho0_),
      alpha(alpha_),
      beta(beta_)
                ...
                if (tag>=0) {
                    ...
                    if (!info.isFrozen()) {
                        ...
                        if (alpha < (T) 0 || beta < (T) 0) {
                            // Model: rho = rho0 * V0/V
                            rho = rho0 * volumeRatio;
                        } else {
                            // Model: rho = rho0 * [1 + alpha * (1 - V/V0)^beta]
                            rho = rho0 * (1.0 + alpha * pow((1.0 - 1.0/volumeRatio), beta));
                        }
                        // Cutoff value
                        if (rho>1.05*rho0) {
                            rho = 1.05*rho0;
