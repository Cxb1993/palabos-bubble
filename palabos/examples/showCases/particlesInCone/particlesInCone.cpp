    ...
    virtual void processGenericBlocks(Box3D domain, std::vector<AtomicBlock3D*> blocks)
    {
        ...
        for (pluint iParticle=0; iParticle<particles.size(); ++iParticle) {
            ...
            for (pluint iNeighbor=0; iNeighbor<neighbors.size(); ++iNeighbor) {
                neighbors[iNeighbor]->getVelocity() = Array<T,3>(0.,0.,0.);
            }
            for (pluint iNeighbor=0; iNeighbor<neighbors.size(); ++iNeighbor) {
                Particle3D<T,Descriptor>* particle = neighbors[iNeighbor];
                Array<T,3> newPosition;
                particle->getVelocity() = newPosition - particle->getPosition();
                particleField.advanceParticles(neighborhood);
            }
