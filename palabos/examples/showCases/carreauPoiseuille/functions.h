...
// 			
			...
			if (hypergeomRef != T())
			{
				diff = fabs((hypergeom-hypergeomRef)/hypergeomRef);
			}
			else 
			{
				diff = fabs(hypergeom-hypergeomRef);
			}
...
public :
	...
	: nu0(nu0_), lambda(lambda_), n(n_), kappa(kappa_), ly(ly_)
	  ...
	  virtual T operator()(T y, T z) const 
	  {
		  return (ly-2*y)*kappa+2*pow(1+lambda*lambda*z*z,(n-(T)1)/(T)2)*nu0*z;
	  }
	  ...
	  virtual T derivative(T y, T z) const
	  {
		  ...
		  return (T)4*pow((T)1+lz2,(n-(T)1)/(T)2)*((n-(T)1)/(T)2)*lz2*nu0/((T)1+lz2)
				  + (T)2*pow((T)1+lz2,(n-(T)1)/(T)2)*nu0;
	  }
